...
class CountTackle(object):
    ...
    def __countTackle(self, team_name):
        ...
        print('-' * 40 + team_name + 'Estatísticas do Tackball' + '-' * 40)
        print('Total de tackles {} vezes: {}'.format(tackle_cycle.count(), t_c))
        print('Enfrente a bola para pegar a bola {} vezes： {}'.format(get_shell, g_s_c))
        print('Pá fora {} vezes： {}'.format(out_side, o_s_c))
        print('Pá passe {} vezes： {}'.format(pass_t, p_t_c))
        print('Pá com sucesso {} vezes： {}'.format(shoot, s_c))
        print('Inválido enfrentar {} vezes： {}\n'.format(fail, f_c))
