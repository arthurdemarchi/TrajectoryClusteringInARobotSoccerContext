...
class World:
            ...
            for line in rcl:
                if(int(line.split(',')[0]) >= 1):
                    ...
                    action = {'kick': None, 'dash': None, 'turn': None, 'turn_neck': None, 'change_view': None,
                              'tackle': None, 'attentionto': None, 'say': None, 'pointto': None, 'playmode': None }
    ...
    def currentPlaymode(self, cycle):
        for i in range(0, len(self.playmode)-1):
            if cycle == self.playmode[i][0]:
                return  self.playmode[i][1]
        return "NCJNA"
    ...
    #pakita
    #comentei esta redefinição de método
...
class GameTime:
    ...
    def setTime(self, cycle):
        self.game_time = cycle
